...
function assignmentCard(item){
  return `<article class="admin-assignment"><div class="admin-assignment-top"><h3>${esc(item.officer||'Belum ditetapkan')} · ${esc(item.route)}</h3><span class="admin-status ${item.status==='confirmed'?'confirmed':''}">${esc(statusName[item.status]||item.status)}</span></div><p><span class="${item.employmentType==='RELIEVER'?'badge-reliever':'badge-pkwt'}">${esc(item.employmentType||'PKWT')}</span> ${esc(item.vendor||'')} · ${esc(item.date)} · ${esc(item.shift)} · Unit ${esc(item.unit)}</p><p>${esc(item.direction)}${item.replacementFor?` · Menggantikan ${esc(item.replacementFor)}`:''}</p>${item.status!=='cancelled'?`<button type="button" data-cancel="${item.id}">Batalkan penugasan</button>`:''}</article>`;
}
...
function renderAssignments(){
  ...
  const pkwt=items.filter(item=>item.employmentType!=='RELIEVER');
...
function renderRelievers(){
  ...
  $('#reliever-list').innerHTML=people.length?people.map(person=>{
    ...
    return `<article class="admin-assignment"><div class="admin-assignment-top"><h3>${esc(person.name)} · ${esc(person.nik)}</h3><span class="badge-reliever">RELIEVER</span></div><p>${esc(person.vendor)} · Area: ${esc(person.areas||'Semua')}</p><p>${availability.length} ketersediaan mendatang · ${stats.completed} penugasan dikonfirmasi · ${stats.declined} penawaran ditolak</p></article>`;
  }).join(''):'<div class="blank-list">Belum ada reliever terdaftar pada perangkat ini.</div>';
}
...
function offerAssignment(relieverId){
  ...
  assignments.push({id:uid('assignment'),employmentType:'RELIEVER',relieverId:person.id,officer:person.name,nik:person.nik,vendor:person.vendor,date:currentNeed.date,route:currentNeed.route,unit:currentNeed.unit,shift:currentNeed.shift,direction:currentNeed.direction,area:currentNeed.area,replacementFor:currentNeed.replacementFor,replacementReason:currentNeed.reason,status:'offered',createdAt:new Date().toISOString(),offerExpiresAt:new Date(Date.now()+10*60*1000).toISOString()});
