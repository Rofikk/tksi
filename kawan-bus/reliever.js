...
function renderOffers(){
  const all=load(ASSIGNMENT_KEY);let changed=false;
  all.forEach(item=>{if(item.status==='offered'&&item.offerExpiresAt&&new Date(item.offerExpiresAt)<=new Date()){item.status='expired';item.expiredAt=new Date().toISOString();changed=true}});
  if(changed)save(ASSIGNMENT_KEY,all);
  const offers=all.filter(item=>item.relieverId===activeId&&['offered','confirmed','declined','expired'].includes(item.status)).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
  $('#offer-list').innerHTML=activeId?(offers.length?offers.map(item=>`<article class="offer-card"><div class="admin-assignment-top"><h2>${esc(item.route)} · ${esc(item.shift)}</h2><span class="admin-status ${item.status==='confirmed'?'confirmed':''}">${item.status==='offered'?'Menunggu jawaban':item.status==='confirmed'?'Diterima':item.status==='expired'?'Kedaluwarsa':'Ditolak'}</span></div><p>${esc(item.date)} · Unit ${esc(item.unit)} · ${esc(item.direction)}</p><p>Menggantikan ${esc(item.replacementFor)} · ${esc(item.replacementReason)}</p>${item.status==='offered'?`<p>Konfirmasi penawaran dalam 10 menit setelah diterima dari Admin Vendor.</p><div class="offer-actions"><button class="primary-button" type="button" data-accept="${item.id}">Terima tugas</button><button class="decline-button" type="button" data-decline="${item.id}">Tidak tersedia</button></div>`:''}</article>`).join(''):'<div class="blank-list">Belum ada jadwal atau penawaran. Data kesiapan Anda tetap tercatat sebagai masukan Admin Vendor.</div>'):'<div class="blank-list">Simpan profil terlebih dahulu untuk melihat penawaran.</div>';
...
$('#profile-form').addEventListener('submit',event=>{
  ...
  save(RELIEVER_KEY,people);renderAvailability();renderOffers();alert('Profil Buffer tersimpan.');
});
...
$('#availability-form').addEventListener('submit',event=>{
  ...
  if(!person){alert('Simpan profil Buffer terlebih dahulu.');return}
